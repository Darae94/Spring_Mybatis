...
<script type="text/javascript">
	...
	function init(continent,nation) {
		...
		for(var i=0; i<continentArr.length; i++) {
			f.continent[i+1] = new Option(continentArr[i], continentArr[i]);
...
<c:set value="<%=continentList%>" var="clist" />
<c:set value="<%=nationList%>" var="nlist" /> --%>
<!-- 
<body>
1. load시 init 실행
2. 자바코드 및 core코드 사용
...
<body onLoad="init('${ movie.continent }','${ movie.nation }')">
	...
	<h2>영화 정보 수정 화면</h2>
	<form:form commandName="movie" name="f" action="update.mv" method="post">
		...
		<p>
			...
			<select name="continent" onChange="firstChange()">
				...
				<%-- <c:forEach var="i" begin="0" end="${ fn:length(clist)-1 }">
					...
					<c:set value="${ i }" var="cIdx" />selected</c:if>
					> ${ clist[i] }
				</c:forEach> --%>
			</select>
			...
			<select name="nation">
				...
				<%-- <c:forEach var="i" begin="0" end="${ fn:length(clist)-1 }">
					<c:if test="${ movie.continent == clist[i] }">
						<c:forEach var="j" begin="0" end="${ fn:length(nlist[i])-1 }">
							<option value="${ nlist[i][j] }" <c:if test="${ nlist[i][j] == movie.nation }">selected</c:if>> ${ nlist[i][j] }
						</c:forEach>
					</c:if>
				</c:forEach>
				<c:forEach var="i" begin="0" end="${ fn:length(nlist[cIdx])-1 }">
					<option value="${ nlist[cIdx][i] }" <c:if test="${ nlist[cIdx][i] == movie.nation }">selected</c:if>> ${ nlist[cIdx][i] }
				</c:forEach> --%>
			</select>
